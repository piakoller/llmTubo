    ...
    def respond(self, diagnostik_summary_context: str) -> tuple[str, str]:
        ...
        template = """
            ...
            **Anweisungen für die Erstellung der Therapieempfehlung:**
            ...
            7.  Gib **ausschließlich** die Therapieempfehlung und die Begründung auf Deutsch aus. Verwende dabei die unten angegebene Struktur der Überschriften. Füge keine zusätzlichen einleitenden oder abschliessenden Sätze hinzu, die nicht Teil der Empfehlung oder Begründung sind.
        ...
        invoke_llm_result_tuple = self._invoke_llm(
            template,
            {
                ...
                "context": diagnostik_summary_context
            }
        )
        logger.debug(f"TherapieAgent _invoke_llm result: (response_snippet='{invoke_llm_result_tuple[0][:50]}...', interaction_id='{invoke_llm_result_tuple[1]}')")
